...
#to use later for name of columns
day <- read.csv("/Users/grisv/GitHub/Manifest/R code/sample_day_filtered.csv")

day <- read.csv("/Users/grisv/GitHub/Manifest/R code/sample_day_filtered.csv", col.names = seq_len(ncol(day)))
light <- read.csv("/Users/grisv/GitHub/Manifest/R code/faulty_light_filtered.csv", col.names = seq_len(ncol(day)))
grip = read.csv("/Users/grisv/GitHub/Manifest/R code/degrading_grip_filtered.csv", col.names = 1:ncol(day))	
lg = read.csv("/Users/grisv/GitHub/Manifest/R code/faulty_light_degrading_grip_filtered.csv", col.names = 1:ncol(day))

#ERROR 1 - data is import with the first column as the names of the columns, no data
# Solved
...
gettrends <- function(data, len){
    ...
    # ERROR - x is not one time increment
...
# Get standard deviations and assign limits (2,2.5,2.5)
...
# number of rows in trends matrix
l = dim(historictrends)[1]
# Rest from i+1 row minus i row, for columns (2-4) light,fricc,gripper
...
# --- Section: Violation Map Calculation ---
...
vmap = read.csv("safezone.csv", header = T)
...
# It creates a new data frame with the first 10 rows of the data and the rest of the data

# add "len" normal datapoints to beginning of new data (dataplus)
start = dim(day)[1] - len+1   #e.g., day has 1000 rows, len = 10, start = 991
end = dim(day)[1]             #e.g., day has 1000 rows, len = 10, end = 1000
extra = day[start:end,]
print(extra)
print("f")
...
# ERROR IN plots as time is not updated in dataplus when adding extra data (so added randonmly between 6600-7140 sec)
# --  HOWEVER it does not affect the trends, as the linear model does not consider the time!!!! 
# check plots below, the third is more noticeable
...
# NO idea for what... I guess tests
...
checktrends <- function(data, vmap, tstep = 60, trendlims, changelims, len=10,
                        maxtime=6000000, mintime, msteps) {
    full_length <- dim(data)[1] # data length = 249

    # assume starting trends are all zero
    keeptrends = rep(0, 3)      # previous trend
    trends = rep(0, 3)          # track slopes of each env. at every time step
    trendchanges = rep(0, 3) # = (0,0,0)
    ...
    #for all data, but starting after len rows
    for (i in (len+1):full_length){

         #for each env. param light,fric,gripper
         for (j in 1:3){

            # Get model for each data row "i"
            #ERROR - does x has to be the time increment?, i.e. 60,120,180,240,300... and not 1,2,3,4,5...
            #= it is ok as treathed as number of rows, not as time 
             model = lm(data[(i-len):(i-1), (j+1)] ~ x)

             trends[j] =  model$coefficients[2] # slope
             trendchanges[j] = abs(trends[j] - keeptrends[j])
         }

         #ERROR - what if the trend observe is within normal values but I am already in violation??
         # check whether trends exceed limits, i.e., there has to be a big change (slope>>limit in slope)
         # FIRST DERIVATIVE = Trend (i.e., change Y/change X)
         if ((abs(trends[1]) > trendlims[1]) | (abs(trends[2]) > trendlims[2]) |(abs(trends[3]) > trendlims[3]))  {
                  
                  # save which ones changed
                  change = rep("none",3)
                  ...
                  # check whether this is the same trend as measured before
                  # i.e., if the trend increment is lower than a threshold 
                  # THIS IS THE SECOND DERIVATIVE!!! THE CHANGE OF THE CHANGE!!!!
                  if ((trendchanges[1] < changelims[1]) & (abs(trendchanges[2]) < changelims[2]) &(abs(trendchanges[3]) < changelims[3]))  {
                           # this is the time to expected failure- time step
                           keept = keept-tstep
                           #ERROR?? - what happen is not a violation but an out of ODD was predicted?
                           #Minor ERROR- what happen at the beginning as the predicted time is actually the total time
                           print(c(time[i], "same trends", " new time (to end, violation or edge): ", keept, "seconds." ))
                           output = rbind(output, c(data[i,1:4], "same trend", keept, change, " "))
                  }
                  # i.e., if the trend increment change is HIGHER than a threshold 
                  # ACCELERATION in the env. variable!!!!!
                  else {
                         # calculate time to violation/edge of parameter space from current point
                         cp = c(data[i, 2], data[i, 3], data[i, 4]) #current environment
                         t = violationtime(cp, vmap, trends, tstep, maxtime) # time to violation
                         if (t[[1]] < maxtime) {
                               keept = t[[1]]
                         ...
                         #ERROR - starting at keept=maxtime=6,000,000
                         # the next iterations, lets say that no time is in violation
                         # hence keept is not uplodaded,
                         # hence keept=6,000,000
                         # imagine that after 5,000,000 time a change in trends happen
                         # 
